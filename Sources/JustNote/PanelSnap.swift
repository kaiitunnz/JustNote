...
/// to a target when it lands within `threshold`, each component independently. Kept free of window
/// state so it is unit-testable; the callers own the per-gesture "already snapped" tracking that
/// drives haptics.
enum PanelSnap {
    ...
    static func snapSize(_ size: NSSize, to target: NSSize, threshold: CGFloat = threshold)
        ...
        let width = snap(size.width, to: target.width, threshold: threshold)
        let height = snap(size.height, to: target.height, threshold: threshold)
    ...
    static func snapOrigin(_ origin: NSPoint, windowSize: NSSize, in visible: NSRect,
        ...
        let x = snap(origin.x, to: centeredX, threshold: threshold)
        let y = snap(origin.y, to: centeredY, threshold: threshold)
        return (NSPoint(x: x.value, y: y.value), x.snapped, y.snapped)
    }

    private static func snap(_ value: CGFloat, to target: CGFloat, threshold: CGFloat)
        -> (value: CGFloat, snapped: Bool) {
        abs(value - target) <= threshold ? (target, true) : (value, false)
    }
